...
# Headers robusti con Accept-Language forzato in Italiano
HEADERS = {
    'x-requested-with': 'WebAppSPA',
    'Accept': 'application/json',
    'x-purpose': 'navigation',
    'Referer': 'https://www.primevideo.com/livetv',
    'Accept-Encoding': 'gzip',
    'Accept-Language': 'it-IT,it;q=0.9,en-US;q=0.8,en;q=0.7', # <--- FORZA LA LINGUA ITALIANA
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
}

def fetch_json(url):
    """Scarica e decomprime il JSON dall'API."""
...
def load_existing_epg(cutoff_dt):
    """Carica l'XML precedente, mantenendo solo i programmi più recenti del cutoff (2 giorni)."""
    channels = {}
    programmes = {}
    ...
    try:
        ...
        print(f"Avviso: impossibile leggere l'EPG esistente (verrà ricreato). Errore: {e}")
...
def generate_epg():
    ...
    print("Inizio download nuovi dati da Prime Video...")
        ...
        for container in containers:
            ...
            for entity in container.get('entities', []):
                ...
                for prog in station.get('schedule', []):
                    ...
                    # Creazione del nodo XML
                    prog_xml = [f'  <programme start="{start_str}" stop="{end_str}" channel="{ch_id}">']
                    prog_xml.append(f'    <title>{main_title}</title>')
                    
                    if sub_title: 
                        prog_xml.append(f'    <sub-title>{sub_title}</sub-title>')
                    if desc: 
                        prog_xml.append(f'    <desc>{desc}</desc>')
                    if img:  
                        prog_xml.append(f'    <icon src="{img}" />')
                    if rating: 
                        prog_xml.append(f'    <rating><value>{rating}</value></rating>')
                    if is_live:
                        prog_xml.append(f'    <category>Live</category>')
                        
                    prog_xml.append('  </programme>')
                    
                    programmes_dict[(ch_id, start_str)] = '\n'.join(prog_xml)
        
        # Gestione paginazione via API
    ...
    print(f"Salvataggio in corso... ({len(channels_dict)} canali, {len(programmes_dict)} programmi totali)")
    ...
    print(f"Finito! File generato con successo: {OUTPUT_FILE}")
